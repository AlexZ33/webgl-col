...
const entryNames = glob
  .sync(path.resolve(__dirname, '../src/packages/*/index.js'))
  .map((fileName) => fileName.match(/\/src\/packages\/(\S*)\/index\.js/)[1]);
const entries = entryNames.reduce(
  (prevObj, entryName) => ({
    ...prevObj,
    [`${entryName}/index`]: path.resolve(__dirname, `../src/packages/${entryName}/index.js`),
  }),
  {}
);
